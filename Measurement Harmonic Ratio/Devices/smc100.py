...
class SMC100(object):
  ...
  def get_software_limt(self):
    Sl = float(self.sendcmd('SL', '?', expect_response=True, retry=10))
    Sr = float(self.sendcmd('SR', '?', expect_response=True, retry=10))
    return Sl,Sr  
  
  def get_encoder_incr(self):
    return float(self.sendcmd('SU', '?', expect_response=True, retry=10))
  ...
  def sendcmd(self, command, argument=None, expect_response=False, retry=False):
    ...
    while self._port is not None:
      ...
      linern = '\r\n'
      self._port.write(linern.encode())
