...
import numpy as np
import threading
import HarmonicRatio_acquire_new as fR
#import HarmonicRatio_testthread as fR
...
class GUI(tk.Frame):
	...
	def setup(self):
		colortheme = 'gray90'
		self.parent.title('Harmonic Ratio Measurement')
		self.parent.configure(background = colortheme)
		self.parent.geometry('1000x820')
		self.parent.title ('Harmonic Ratio Measurement') # give
		...
		self.ax2.set_title('Harmonic Ratio')
		self.ax2.set_yscale('log')
		self.ax2.grid()
		self.ax2.set_xlabel('Peak Int [W/m**2]')
		self.ax2.set_ylabel('fR [a.u.]')
		self.ax2.set_xlim(0,1e3)
		self.ax2.set_ylim(1e-1,1e6)
		...
		#self.OPAwavelength.insert('end', 'WAVELENGTH (nm)')
		...
		#self.OPAwavelength.config(state='disabled')

		#self.OPAsetting = tk.Text(height=1,width=15, bg = colortheme)
		#self.OPAsetting.place(x = 850, y = 25)"""
		...
		self.NumDataPointsText.place(x = 850, y = 300)
		...
		self.HysteresisButton = tk.Button(self.parent, text = 'Hysteresis', font = ('calibri',12), bg='red', command = lambda: self.changeHysteresisStatus())
		self.HysteresisButton.place(x = 850, y = 700)
		self.HysteresisButton.config(width = 15,height = 2)
	...
	def plot_start(self):	
		...
		if self.InputParamCond == True:
			...
			#Meas_thread = threading.Thread(target = fR.startMeasurement, args = (self, *self.read_input_fields(), self.TOPAS, self.SPECTROMETER, self.ROTATIONSTAGE,))
			Meas_thread = threading.Thread(target = fR.start_measurement, args = (self, self.ROTATIONSTAGE, self.TOPAS, self.SPECTROMETER,))
			Meas_thread.start()	
		else:
	...
	def disable_buttons(self):
		...
		self.HysteresisButton.config(state="disabled")		
	...
	def enable_buttons(self):
		...
		self.HysteresisButton.config(state="normal")		
	...
	def read_input_fields(self):
		...
		try:
			...
			if self.DispersionStatus:
				self.setDispersionRange()
				print("Set wavelengths for dispersion measurement (nm): ", self.disp_wavelengths)
	...
	def changeHysteresisStatus(self):
		self.HysteresisStatus = not self.HysteresisStatus
		if self.HysteresisStatus:
			self.HysteresisButton.config(bg = 'green')
		else:
			self.HysteresisButton.config(bg = 'red')
