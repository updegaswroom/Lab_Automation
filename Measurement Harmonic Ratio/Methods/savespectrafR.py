...
def savespectrafR(ID = None, Inttime = None, OPALambda = None, Power = None,Wavelengths = None, Intensities = None, DarkSpectra = False, silent = True):
		...
		checkpath = os.path.join(cwd,"Harmonic Ratio Measurements",date,ID,str(OPALambda))
		if not directory.check_for_dir(checkpath,silent):
			pathmeas = directory.create_new_dir(cwd,"Harmonic Ratio Measurements")
		...
		filesuffix = ".txt"
		if DarkSpectra:
			filename = f"HarmRatio_{sampleID}_{timestamp}_{str(power)}_mW_dark{filesuffix}"
		else:
			filename = f"HarmRatio_{sampleID}_{timestamp}_{str(power)}_mW{filesuffix}"
